...
/* 
Extract just the customers' contact email addresses and store
them in a new array. You will need a nested array method -
meaning one iteration inside another one - since you need to
iterate the entire array of customers, and then iterate the
array of emails for each one.
*/

let newCustomers = [];

for ( i = 0; i < customers.length; i++) {
    for (j = 0; j < customers[i].contacts.email.length; j++) {
        newCustomers.push(customers[i].contacts.email[j]);
    }
}

const newCustomerEmailArray = customers.map(cust => cust.contacts.email).flat(5);

const newCustomerEmailArrays = customers.reduce((acc, crnt) => acc.concat(crnt.contacts.email), []);


console.log(newCustomers);
console.log(newCustomerEmailArray);
console.log(newCustomerEmailArrays);

// console.log(newCustomerEmailArray.flat());


// const newCustomers = customers.map(customer => customer.contacts.email.forEach(email => splitEmails(email)));
